...
int o=0,a[N+2][N+2],s,f[9][600],g[9];
...
int MT = 0; //有无魔毯
int LP = 10; //我的小屋路牌个数
int HYB = 10; //sonic合影板个数
int DZ = 13; //音阶地砖个数（不影响布局，铺满为169）
int XK = 0; //有无新秀相框（不影响布局）

int m = 153062; // 只搜索这个数值及以上的方案
int skip = 0; // 为降低复杂度，可以先skip=1跳过1*1多于10个的情况，将得出的最高值代入上一行的m，再skip=0运行一遍以保证完备性
...
int main()
{
    ...
    for(n[0]=min(GJ,(N*N-80-64*n[8]-49*n[7]-36*n[6]-25*n[5])/20);n[0]>=0;n[0]--) // 冠军之路
    ...
    for(n[3]=(N*N-80-64*n[8]-49*n[7]-36*n[6]-25*n[5]-20*n[0]-16*n[4])/9;n[3]>=0;n[3]--)
    {
        ...
        for(n[2]=min(MG+2*YL+XX+1,(N*N-80-64*n[8]-49*n[7]-36*n[6]-25*n[5]-20*n[0]-16*n[4]-9*n[3])/4);n[2]>=0;n[2]--)
        {
            ...
            if(skip && n[1]>10) continue;
            int ff=9400+4360+4394+33648-148*(1-XK)-2*(169-DZ);//小屋+背景+地砖+室内（20230->26944->33648）
