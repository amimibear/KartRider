// by 啊咪咪小熊
// fix 4*5的冠军之路在左上角
// 之前xiaowu45不完备，暂时只能用这个，因为4*5的用武之地主要在左上角，要想完备必须加入长方形搜索
#include <iostream>
#include <fstream>
using namespace std;
#define N 26//小屋边长
...
int LP = 0; //我的小屋路牌个数（默认足够）
int HYB = 0; //sonic合影板个数（默认足够）
...
int main()
{
    ...
    // for(int i=1;i<600;i++) f[7][i]=0; f[7][1]=7800; f[7][2]=6600; // 2 7*7 无怪诞
    ...
    for(n[6]=o6?(N*N-100-64*n[8]-49*n[7])/36:min(1+FJ+JT,N*N-100-64*n[8]-49*n[7]);n[6]>=(o6?min(1+FJ+JT,N*N-100-64*n[8]-49*n[7])+1:0);n[6]--)//先从1+FJ+JT搜，再从最大搜
    for(n[5]=(N*N-100-64*n[8]-49*n[7]-36*n[6])/25;n[5]>=0;n[5]--)
    for(n[4]=(N*N-100-64*n[8]-49*n[7]-36*n[6]-25*n[5])/16;n[4]>=0;n[4]--)
    for(n[3]=(N*N-100-64*n[8]-49*n[7]-36*n[6]-25*n[5]-16*n[4])/9;n[3]>=0;n[3]--)
    {
        int o=1,oo=1;
        for(n[2]=min(MG+2*YL+XX+1,(N*N-100-64*n[8]-49*n[7]-36*n[6]-25*n[5]-16*n[4]-9*n[3])/4);n[2]>=0;n[2]--)
        {
            n[1]=N*N-100-64*n[8]-49*n[7]-36*n[6]-25*n[5]-16*n[4]-9*n[3]-4*n[2];
            // if(n[1]>=10) continue;
            int ff=3360+9400+4360+4394+20230-148*(1-XK)-2*(169-DZ);//小屋+背景+地砖+室内
            ...
            if(ff>=m&&oo)// >
            {
                ...
                {
                    o=0;
                    g[1]=N*N-100-64*n[8]-49*n[7]-36*n[6]-25*n[5]-16*n[4]-9*n[3];
                    g[2]=0;
